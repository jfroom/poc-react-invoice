...
const InvoiceList = (props) => {
  const { invoices, statusFilter, handleDeleteInvoice, handleChangeStatusFilter } = props
  ...
  const getInvoiceRow = (invoice) => {
    ...
    const overdue =
      invoice.status === STATUS_TYPE.UNPAID && new Date(invoice.date) - new Date() <= 0
...
InvoiceList.propTypes = {
  statusFilter: PropTypes.oneOf(['', STATUS_TYPE.UNPAID, STATUS_TYPE.PAID]).isRequired,
  ...
  handleChangeStatusFilter: PropTypes.func.isRequired,
}
InvoiceList.defaultProps = { statusFilter: '' }
